...
using NPCArmorDistributor.Utils;
using NPCArmorDistributor.Armor;
using NPCArmorDistributor.Config;
using Mutagen.Bethesda.Plugins;
using System.Text;
using log4net;

namespace NPCArmorDistributor.Managers
{
    public class OutfitManager
    {
        ...
        private void MergePatchedOutfits()
        {
            ...
            var outfits = State.LoadOrder.PriorityOrder
                ...
                .Where(o => !Program.Settings.User.ModsToSkip.Contains(o.FormKey.ModKey)
        ...
        private void CreateArmorsSets()
        {
            ...
            var modlists = State.LoadOrder.PriorityOrder
                .Where(x => (Program.Settings.User.ArmorMods.ContainsKey(x.ModKey.FileName)
            ...
            for (int l = 0; l < modlists.Count(); l++)
            {
                ...
                var modsCategories = Program.Settings.User.ArmorMods[mod.ModKey.FileName].Distinct().ToList();
                ...
                var commanName = 0;
                if (bodyCount > 5 && Program.Settings.User.ArmorMods[mod.ModKey.FileName].Contains("Generic"))
                ...
                for (int i = 0; i < bodyCount; i++)
                {
                    ...
                    bMats.ForEach(bMat =>
                    {
                        ...
                        if (Program.Settings.User.DistributeWeapons && weapons.Any())
        ...
        {
            if (!Program.Settings.User.ResolveOutfitConflicts) return;
            ...
            foreach (var outfit in State.LoadOrder.PriorityOrder
                .Where(x => Program.Settings.User.ArmorMods.ContainsKey(x.ModKey.FileName.String))
                .WinningOverrides<IOutfitGetter>())
            {
                ...
                if (winningOtfts.Any())
                {
                    ...
                    var overridenOtfts = context.Where(c => Program.Settings.User.ArmorMods.ContainsKey(c.ModKey.FileName.String)).ToList();
                    var lastNonModOutfit = context.Where(c => !Program.Settings.User.ArmorMods.ContainsKey(c.ModKey.FileName.String)).ToList();
                    ...
                    if (overridenOtfts.Count > 0 && lastNonModOutfit.Count > 1)
                    {
                        ...
                        Patch = FileUtils.GetIncrementedMod(Patch);
                        LeveledItem sLL = LeveledListUtils.CreateLeveledList(Patch, oLLs.Distinct(), "sll_" + outfit.EditorID, 1, Program.Settings.LeveledListFlag);
                        nOutfit.Items = new();
        ...
        {
            if (Program.Settings.User.AssignMannequinOutfits)
            {
        ...
        private void CreateSPID()
        {
            ...
            var percentage = Program.Settings.User.OutfitDistributionPercentage;
            var cache = State.LoadOrder.ToMutableLinkCache();
            ...
            foreach (var pair in GrouppedArmorSets)
                ...
                .ForEach(x =>
                {
                    ...
                    if (Program.Settings.User.SkipGuardDistribution && pair.Key.StartsWith("Guards")) line2 = ";" + line2;
                    if (armorSets < 1) line2 = ";" + line2;
                    if (Program.Settings.User.FilterUniqueNPC) SPID.Add(line2.Replace("/U|NONE", "|NONE"));
                    SPID.Add(line1);
                    SPID.Add(line2);
        ...
        {
            if (!Program.Settings.User.SkipGuardDistribution) return;
