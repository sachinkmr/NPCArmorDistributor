...
namespace NPCArmorDistributor.Armor
{
    public class TArmorGroup
    {
        ...
        {
            var cache = Program.Settings.State.LoadOrder.ToMutableLinkCache();
            ...
            foreach (var g in gArmors.Keys)
            {
                ...
                if (Program.Settings.Cache.TryResolve<IOutfitGetter>(OutfitUtils.GetOutfitName(eid), out var ot))
                {
                    newOutfit = Patch.Outfits.GetOrAddAsOverride(ot);
                    LeveledItem mLL = LeveledListUtils.CreateLeveledList(Patch, set, "mLL_" + eid, 1, Program.Settings.LeveledListFlag);
