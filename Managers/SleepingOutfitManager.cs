...
namespace NPCArmorDistributor.Managers
{
    public class SleepingOutfitManager
    {
        ...
        public ISkyrimMod Process(ISkyrimMod patch)
        {
            ...
            if (!Program.Settings.User.SleepingOutfit.Any())
            {
            ...
            foreach (var mod in State.LoadOrder.PriorityOrder
                        .Where(x => Program.Settings.User.SleepingOutfit.Contains(x.ModKey.FileName)
                ...
                for (int i = 0; i < bodyCount; i++)
                {
                    ...
                    bMats.ForEach(bMat => {
                        ...
                        if (armorSet.Armors.Count() == 1)
                        {
                            armorsets.Add(body);
                            if (i > 0 && (i + 1) % 100 == 0)
                                Console.WriteLine("Created {0}/{1} sleeping set for: {2}", i + 1, bodyCount, mod.ModKey.FileName);
                        }
                    });
                }
                Console.WriteLine("Created {0}/{0} sleeping armor-set for: {1}", bodyCount, mod.ModKey.FileName);
            }
            Console.WriteLine("Created {0} matching sleeping armor sets from armor mods...\n", armorsets.Count());
        ...
        private bool HasMod(string modFile) {
            ...
            {
                Console.WriteLine("Mod \"{0}\" not found...", modFile);
                return false;
            }
            return true;
        }
        ...
        private void AddToFormList(List<IArmorGetter> armorsets, string eid, string modFile)
        {
            ...
            record.Items.Clear();
            record.Items.AddRange(armorsets.Select(x => x.ToLink()));
        }
            ...
            armorsets.Select(x => cache.Resolve<IArmorGetter>(x.FormKey))
                ...
                .ForEach(a=> {
                    if (a.WorldModel.Male == null)
                    {
                        ...
                        addon.WorldModel.Male.File = meshes.ElementAt(i).RawPath;
                    }
                });
        }
    }
